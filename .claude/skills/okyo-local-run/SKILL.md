...
- `./run` — convenience script, but **WARNING: it hardcodes `/Users/rober/Documents/Okyo-1`, a stale path**. Do not trust stale absolute paths anywhere (worktrees move: Desktop checkout, conductor workspaces). Always resolve from the current session's repo root.
...
- Trusting `./run`'s hardcoded `/Users/rober/Documents/Okyo-1` path or any stale absolute path.
